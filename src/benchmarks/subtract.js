(function(log) {
  ...
  for(i = 0; i < repetitions; i++) {
    a[Math.floor(Math.random() * a.length)] = Math.round();
    b[Math.floor(Math.random() * b.length)] = Math.round();
    res = vector.subtract(a, b);
  }
  ...
  for(i = 0; i < repetitions; i++) {
    ...
    res = vectorious.Vector.subtract(a, b);
  }
